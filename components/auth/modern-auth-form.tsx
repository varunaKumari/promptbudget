...
export function ModernAuthForm({ mode }: ModernAuthFormProps) {
  ...
  return (
    <div className="w-full max-w-[410px] rounded-[24px] border border-white/10 bg-white/[0.045] p-4 text-white shadow-[0_24px_90px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:p-5">
      <div className="mb-4 flex flex-col items-center text-center">
        <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl border border-white/12 bg-white/[0.06] shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
          <Logo size="lg" showText={false} href="" />
        </div>
        <h1 className="text-2xl font-semibold tracking-normal sm:text-3xl">{copy.heading}</h1>
        <p className="mt-1 text-sm font-medium text-white/54">{copy.subheading}</p>
      </div>
        ...
        {oauthButtons.map((item) => {
          ...
          return (
            <button
              ...
              className="flex h-9 items-center justify-center gap-3 rounded-xl border border-white/10 bg-white/[0.055] px-4 text-sm font-semibold text-white transition-all hover:border-white/20 hover:bg-white/[0.09] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/70 disabled:pointer-events-none disabled:opacity-60"
            >
        ...
        {iconProviders.map((item) => {
          ...
          return (
            <button
              ...
              disabled={disabled}
              onClick={() => handleOAuth(item.provider)}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-white/68 transition-all hover:-translate-y-0.5 hover:border-white/22 hover:bg-white/[0.08] hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400/70 disabled:pointer-events-none disabled:opacity-60"
      ...
      <div className="my-3 grid grid-cols-[1fr_auto_1fr] items-center gap-4 text-xs font-medium text-white/38">
      ...
      <form onSubmit={handleEmailAuth} className="space-y-2.5">
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-white/58">Email</span>
          <span className="flex h-10 items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4 transition-all focus-within:border-blue-400/70 focus-within:ring-4 focus-within:ring-blue-500/10">
            <Mail className="h-4 w-4 text-white/36" />
            <input
              type="email"
              autoComplete="email"
              required
              ...
              className="h-full min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/28"
        ...
        <label className="block">
          <span className="mb-1.5 block text-xs font-semibold text-white/58">Password</span>
          <span className="flex h-10 items-center gap-3 rounded-xl border border-white/10 bg-black/20 px-4 transition-all focus-within:border-blue-400/70 focus-within:ring-4 focus-within:ring-blue-500/10">
            <LockKeyhole className="h-4 w-4 text-white/36" />
            <input
        ...
        {!isSignup && (
          <button
            ...
            className="text-sm font-medium text-blue-300 transition-colors hover:text-blue-200 disabled:pointer-events-none disabled:opacity-60"
          >
      ...
      {isSignup ? (
        <>
          ...
          <p className="mt-4 text-center text-sm text-white/52">
            ...
            <Link href="/login" className="font-semibold text-blue-300 transition-colors hover:text-blue-200">
              Log in
            </Link>
          </p>
        </>
        ...
        <p className="mt-4 text-center text-sm text-white/52">
          ...
          <Link href="/signup" className="font-semibold text-blue-300 transition-colors hover:text-blue-200">
            Sign up
          </Link>
        </p>
      )}
